Here are self-contained prompts, one per session, meant to be pasted into a fresh Agent-mode conversation each time, in order. **Execution order for Sessions 4+:** 4a (done) → **4c** → **4d** → 4b → 5. Each one re-points the agent to the actual files (the "source of truth") rather than to chat summaries.
...
Outcome: `docs/calibration_forge/kicad_symbol_kb.md` §Resultado. Do not re-run unless drift check finds regressions. See also `docs/calibration_forge/session_4b_preflight_verification.md` for a confirmed pre-existing config bug (`rag_top_k: 0.95` truncates to `0`) that must be fixed before Session 4b's A/B experiment.
...
### Session 4c — LLM output guardrails & multi-turn recovery (blocks Session 4b)

**Prerequisite for Session 4b.** Full scope: [`docs/calibration_forge/llm_output_pipeline.md`](docs/calibration_forge/llm_output_pipeline.md) and evidence in [`llm_truncation_review_06072026.md`](docs/calibration_forge/llm_truncation_review_06072026.md).
...
### Session 4d — Dual-backend orchestration & harness pipeline (depends on Session 4c P0)

**Recommended before Session 4b.** Full scope: [`docs/calibration_forge/llm_output_pipeline.md`](docs/calibration_forge/llm_output_pipeline.md) §Session 4d.
...
### Session 4b — Prompt vs. RAG rebalancing (depends on Session 4a, **4c P0**, recommended 4d)

**Prerequisites (06-jul-2026):**
- [`docs/calibration_forge/llm_output_pipeline.md`](docs/calibration_forge/llm_output_pipeline.md) — **Session 4c P0 complete**
- Session 4d complete (or explicitly deferred in `llm_output_pipeline.md` §Resultado)
- [`llm_truncation_review_06072026.md`](docs/calibration_forge/llm_truncation_review_06072026.md) — P0 mitigations verified on live runs
