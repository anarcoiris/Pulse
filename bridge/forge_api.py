...
def load_preset(name: str) -> CircuitGraph:
    ...
    presets = {
        "basic_rc": "presets.basic_rc",
        "rlc": "presets.rlc",
        "mcu": "presets.mcu_uart",
        "mcu_uart": "presets.mcu_uart",
        "esp32_usb_devkit": "presets.esp32_usb_devkit",
        "esp32s2_usb_devkit": "presets.esp32s2_usb_devkit",
    }
    if name in presets:
        import importlib
        mod = importlib.import_module(presets[name])
        return mod.load()
