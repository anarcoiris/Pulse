...
def generate_all_manufacturing_files(
    ...
    logger.info("gerber_export", f"generate_all_manufacturing_files() iniciado para {pcb_path.name} -> {out}")

    # P1: DRC obligatorio primero
    results["drc"] = run_drc(cli, pcb_path, out / "reports")
    drc_ok = results["drc"].get("success", False)
    if drc_ok:
        logger.info("gerber_export", f"DRC OK para {pcb_path.name}")
    else:
        logger.warning(
            "gerber_export",
            f"DRC fallo para {pcb_path.name}: {results['drc'].get('error_count', 0)} errores",
        )
    try:
        ...
        record_design_outcome(
            ...
            mcu="",
            drc_violations=results["drc"].get("error_count", 0),
            gerber_path=str(out),
            passed=drc_ok,
            lessons=[
                f"DRC {'passed' if drc_ok else 'failed'} for {pcb_path.name}",
            ],
        )
        logger.info("gerber_export", f"design experience registrada para {pcb_path.stem}")
    except Exception as e:
        logger.error("gerber_export", f"record_design_outcome() fallo para {pcb_path.stem}: {e}")
    if not drc_ok:
    ...
    if not results["gerbers"].get("success", False):
        logger.error("gerber_export", f"export_gerbers() fallo: {results['gerbers'].get('stderr') or results['gerbers'].get('error')}")
    results["drill"]    = export_drill(cli, pcb_path, out / "gerbers")
    if not results["drill"].get("success", False):
        logger.error("gerber_export", f"export_drill() fallo: {results['drill'].get('stderr') or results['drill'].get('error')}")
    results["position"] = export_position(cli, pcb_path, out)
    if not results["position"].get("success", False):
        logger.warning("gerber_export", f"export_position() fallo: {results['position'].get('stderr') or results['position'].get('error')}")
    results["preview"]  = export_svg(cli, pcb_path, out / "preview")
    if not results["preview"].get("success", False):
        logger.warning("gerber_export", f"export_svg() fallo: {results['preview'].get('stderr') or results['preview'].get('error')}")
    ...
    logger.info("gerber_export", f"generate_all_manufacturing_files() finalizado: {results['summary']}")
