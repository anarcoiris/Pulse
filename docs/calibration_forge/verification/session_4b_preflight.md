...
> **Role:** verification  
> **Status:** archived audit (blockers in §7 resolved)  
> **Source of truth for:** pre-4b confounder analysis  
> **Last verified:** 2026-07-07  
> **See also:** [`../pipelines/llm_output_pipeline.md`](../pipelines/llm_output_pipeline.md) · [`../../status/CURRENT_SPRINT.md`](../../status/CURRENT_SPRINT.md)
...
>
> Fuentes primarias re-leídas para esta verificación: `docs/status/CURRENT_SPRINT.md` (antes `CURENT_SPRINT.md`), `Pulse_cfg.json`, `docs/calibration_forge/index.md`, `docs/calibration_forge/prompt_vs_rag_balance.md`, `docs/calibration_forge/kicad_symbol_kb.md`, `knowledge/circuit_synthesizer.py`, `knowledge/semantic_reviewer.py`, `knowledge/rag_engine.py`, `knowledge/design_experience.py`, `knowledge/validate_complex_apps.py`, `knowledge/kicad_schematic_parser.py`, `knowledge/kicad_symbol_parser.py`, `presets/esp32_usb_devkit.py`, `ui/forge_controller.py`, `docs/status/FORGE_STATUS.md`, `docs/architecture/SEGURIDAD_DEPENDENCIAS.md`, `requirements.txt`, `.gitignore`.
...
### Resolución de Session 5 (07-jul-2026)
Todos los ítems de la tabla quedaron resueltos:
- `requirements.txt`: las 15 dependencias fijadas con `==` a las versiones actualmente instaladas y verificadas con `pip list --format=freeze` (`numpy==2.5.1`, `pygame==2.6.1`, `scikit-learn==1.9.0`, etc.).
- `scratch/test_drc_fail.py`: convertido en `tests/test_kicad_drc_integration.py` (assert-based, `pytest.mark.skipif(find_kicad_cli() is None, ...)` para no romper CI sin KiCad instalado) y verificado en verde localmente; el fichero de `scratch/` fue borrado.
- Docs de arquitectura duplicados: fusionados — `docs/Architecture.md` → anexo en `docs/architecture/APP_ARCHITECTURE.md`; `docs/Architecture_violations.md` → anexo en `docs/architecture/ARCHITECTURE_VIOLATIONS.md`; los ficheros raíz quedaron como stubs de redirección.
- Artefactos de path con backslash en `git status`: ya no reproducen (confirmado con `git status --porcelain=v1` limpio de duplicados).
- CI mínimo: añadido `.github/workflows/ci.yml` (matriz Python 3.11/3.12, `pip install -r requirements.txt` + `pytest tests/ -q`).
...
## 7. Resolución de blockers (06-jul-2026, noche)

| Blocker | Estado | Evidencia |
|---|---|---|
| `rag_top_k: 0.95 → int() == 0` | ✅ Resuelto | `Pulse_cfg.json` → `rag_top_k: 1`, `rag_top_k_variant_b: 4`; warning en `_circuit_example_rag_top_k()` |
| Toggle A/B + semantic review en harness | ✅ Resuelto | `--variant`, `SemanticReviewer`, `semantic_review` en manifiestos; `tests/test_ab_variant.py` |
| Experimento A/B 10 corridas | ✅ Ejecutado (confundido) | Runs `182955` (A) y `201754` (B); ver [`prompt_vs_rag_balance.md` §Resultado A/B](./prompt_vs_rag_balance.md) |
| Embeddings densos obsoletos | ✅ Resuelto | `build_embed_index` completado 06-jul noche; manifest actualizado |
| Reviewer truncado (confounder principal) | ✅ 4c P0 verificado live | runs `212059`/`213418`; reviewer JSON OK en 61s/115s |
| Decisión trimming reglas OBLIGATORIAS | ⏸ Diferido | Requiere A/B limpio tras 4c+4d — no decidir con datos confundidos |
