...
\begin{tabularx}{\textwidth}{>{\hsize=0.75\hsize\raggedright\arraybackslash}X >{\hsize=0.85\hsize\raggedright\arraybackslash}X >{\hsize=1.2\hsize\raggedright\arraybackslash}X >{\hsize=1.2\hsize\raggedright\arraybackslash}X r}
\toprule
\textbf{Función} &
\textbf{Banda (MHz)} &
...
\label{tab:comp_sk}
\renewcommand{\arraystretch}{1.25}
\begin{tabularx}{\textwidth}{>{\hsize=0.8\hsize\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
\toprule
...
\label{tab:umbrales}
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\textwidth}{>{\hsize=0.85\hsize\raggedright\arraybackslash}X >{\hsize=1.0\hsize\raggedright\arraybackslash}X >{\hsize=1.15\hsize\raggedright\arraybackslash}X l}
\toprule
...
Para una PFN de $N=5$ secciones: $L_k = \SI{50}{nH}$, $C_k = \SI{20}{pF}$ por sección.
...
\label{tab:comp_hk}
\renewcommand{\arraystretch}{1.25}
\begin{tabularx}{\textwidth}{>{\hsize=0.8\hsize}X X X}
\toprule
...
\subsection{Rediseño del Circuito Teórico: Enfoque en el Control y la Seguridad}

El nuevo diseño se centra en tres pilares:
\begin{itemize}
    \item \textbf{Conmutación Controlada:} Reemplazar el \textit{spark gap} caótico por un sistema de conmutación de estado sólido, controlado y repetible.
    \item \textbf{Gestión de la Energía:} Asegurar que la descarga sea rápida y potente, pero contenida y dirigida.
    \item \textbf{Aislamiento y Seguridad:} Proteger tanto al operador como a la electrónica circundante.
\end{itemize}

\subsubsection{1. Solución a la Conmutación (Problemas Spark Gap, Interruptores)}
El \textit{spark gap} es la mayor fuente de caos. Lo reemplazamos por un tiristor (SCR) de pulso o un IGBT de alta velocidad, que son los componentes estándar en la industria para esta aplicación.

\textbf{Componente Clave:} Un SCR de pulso (como un dispositivo de la familia "Pulse Rated Thyristor") o un IGBT de alta velocidad y alta tensión.

\textbf{Ventajas:}
\begin{itemize}
    \item \textbf{Disparo Preciso:} Se activan con una señal de baja tensión (5--15\,V) en su puerta (\textit{gate}), permitiendo un control por microcontrolador o circuito de temporización.
    \item \textbf{Jitter Casi Nulo:} El momento de la conmutación es altamente predecible y repetible.
    \item \textbf{Baja Resistencia en Conducción:} Una vez que se encienden, se comportan como un cortocircuito, ideal para una descarga de alta corriente.
    \item \textbf{Sin EMI de Conmutación:} A diferencia del arco, no hay una explosión de plasma que genere interferencias caóticas.
\end{itemize}

\subsubsection{2. Solución a la Fuente de Alimentación (Problema Flyback)}
La fuente \textit{flyback} es inadecuada para cargar un banco de condensadores de forma precisa. La reemplazamos por una fuente de alta tensión con control de corriente.

\textbf{Componente Clave:} Módulo fuente HV DC-DC con control de corriente y voltaje (disponibles comercialmente para aplicaciones de láser $\text{N}_2$, tubos de Geissler, etc.).

\textbf{Ventajas:}
\begin{itemize}
    \item \textbf{Salida DC Limpia:} Proporciona un voltaje estable y sin ruido.
    \item \textbf{Control Preciso:} Permite definir la tensión de carga exacta (ej. 4,5\,kV) y limitar la corriente para una carga segura y controlada de los condensadores.
    \item \textbf{Seguridad:} Suelen incluir protecciones internas de sobrecorriente y sobrevoltaje.
\end{itemize}

\subsubsection{3. Solución al Inductor y la Antena (Problema de Layout y Desadaptación)}
...
\textbf{Componente Clave:} Una Red Formadora de Pulso (PFN - \textit{Pulse Forming Network}). Esta es la solución estándar de ingeniería para generar pulsos rectangulares de alta potencia.

\textbf{Diseño de la PFN:}
\begin{itemize}
    \item En lugar de un único LC, usamos una cadena de secciones LC (el documento original lo mencionaba: PFN tipo Guillemin).
    \item Esto crea una línea de transmisión artificial con una impedancia característica $Z_0$ bien definida.
    \item El diseño se basa en la impedancia de la antena (ej. 50\,$\Omega$) y la duración del pulso deseada ($\tau$).
\end{itemize}
\begin{align}
    Z_0 &= \sqrt{\frac{L_{\text{sección}}}{C_{\text{sección}}}} = 50\,\Omega \\
    \tau &= 2N \sqrt{L_{\text{sección}} \cdot C_{\text{sección}}}
\end{align}
(donde $N$ es el número de secciones)

\begin{itemize}
    \item \textbf{Inductores:} Se deben usar inductores de aire o toroides de ferrita específicos para alta potencia de pico, calculados para no saturar con la corriente de pico esperada.
    \item \textbf{Antena:} La antena TEM Horn sigue siendo una buena opción, pero ahora está perfectamente adaptada a la impedancia de la PFN, maximizando la transferencia de energía y minimizando reflexiones.
\end{itemize}

\subsubsection{Esquema del Circuito Mejorado}
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{circuit_redesign.pdf}
\caption{Esquema del Circuito Mejorado: Conmutación Controlada y PFN.}
\end{figure}

\textbf{Descripción de los Componentes Mejorados:}
\begin{itemize}
    \item \textbf{Fuente HV DC-DC Controlada:} Proporciona un DC limpio. S1 es un interruptor de seguridad principal. S2 permite aislar el banco para medir su voltaje de forma segura.
    \item \textbf{Banco de Condensadores (PFN):} En lugar de un simple LC, tenemos un banco de condensadores que forma parte de la PFN. Los valores de C y L están calculados para crear una impedancia de 50\,$\Omega$ y un pulso de, por ejemplo, $\tau = 100\,$ns.
    \item \textbf{Conmutador de Estado Sólido (SCR/IGBT):} Este es el corazón del nuevo diseño. S3 y S4 son la conexión a la carga (la PFN). El dispositivo se activa mediante una señal de control en su puerta (\textit{Gate}), que puede venir de un circuito temporizador simple (555) o un microcontrolador (Arduino, PIC) para una precisión total.
    \item \textbf{Inductor de Aislamiento:} Un pequeño inductor en serie con el conmutador ayuda al aislamiento de alta tensión.
\end{itemize}
...
\label{tab:deteccion}
\renewcommand{\arraystretch}{1.25}
\begin{tabularx}{\textwidth}{>{\hsize=0.6\hsize}X X X X l}
\toprule
\textbf{Método} &
\textbf{Alcance} &
\textbf{Ventajas} &
...
Radar FMCW &
  0,1–5\,km &
  Alta precisión, todo tiempo, activo &
  Requiere licencia, alta complejidad &
  Alto \\[3pt]

Detector RF pasivo &
  0,1–2\,km &
  Sin emisión, detecta enlace RC &
  Ciego si UAV en modo autónomo &
  Medio \\[3pt]

Array de micrófonos &
  50–500\,m &
  Pasivo, sin emisión, bajo coste &
  Afectado por ruido ambiental &
  Bajo \\[3pt]

Visión + IA (EO/IR) &
  50\,m–1\,km &
  Identificación visual &
  Niebla, lluvia, contraluz &
  Medio \\[3pt]

LIDAR 3D &
  50–500\,m &
  Alta resolución punto nube &
  Alcance limitado, lluvia/polvo &
  Alto \\[3pt]

Fusión &
  0,1–3\,km &
  Máxima robustez &
  Coste y complejidad integración &
  Muy alto \\
\bottomrule
\end{tabularx}
\end{table}
...
\renewcommand{\arraystretch}{1.25}
\begin{tabularx}{\textwidth}{>{\hsize=0.6\hsize}X >{\hsize=0.6\hsize}X X}
\toprule
...
ITU Radio Reglamento, Art.~15 &
  Internacional &
  Prohibición expresa de interferencia \\[3pt]
...
Directiva 2014/53/UE (RED) &
  UE &
  Homologación de equipos radio \\[3pt]

EASA Regl. 2019/947 &
  UE &
  Marco de operación de UAVs y C-UAS \\[3pt]

Ley 9/2014 General de Telecom. &
  España &
  Gestión del espectro radioeléctrico y sanciones \\[3pt]

CNAF (Cuadro Nacional de AF) &
  España &
  Asignación; uso de bandas 2,4 / 5,8\,GHz (ISM) \\[3pt]
...
FCC Part 15 / 47 C.F.R. §97 &
  EE.UU. &
  Comercialización en EE.UU.; aprobación FCC \\
\bottomrule
\end{tabularx}
\end{table}
...
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\textwidth}{>{\hsize=0.6\hsize}X l X X}
\toprule
\textbf{Fase} &
\textbf{Duración} &
\textbf{Entregables} &
...
1. Diseño Conceptual &
  3 meses &
  Especificación técnica v1.0; Selección de componentes &
  Validación de arquitectura \\[4pt]

2. Prototipo Lab &
  6 meses &
  PCB RF funcional; Banco condensadores prueba &
  Medición parámetros cámara anecoica \\[4pt]

3. Integración &
  9 meses &
  Sistema integrado en trípode; Apuntado operativo &
  Neutralización efectiva $\geq 80\%$ en $\leq \SI{200}{m}$ \\[4pt]

4. Prototipo Pre-serie &
  6 meses &
  5 unidades; Ensayos MIL-STD-461 &
  Superación ensayos EMC y ambientales \\[4pt]

5. Producción Serie &
  Continuo &
  Lotes 10–50; Soporte técnico &
  Tasa rechazo $< 2\%$; MTBF $> \SI{2000}{h}$ \\
\bottomrule
\end{tabularx}
\end{table}
...
\label{tab:costes}
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\textwidth}{>{\hsize=1.2\hsize}X X X}
\toprule
\textbf{Subsistema} &
...
Módulo RF Soft Kill &
  2.500–4.500 &
  Incluye PCB RF 4 capas, PA MMIC $\times 2$, antenas \\[3pt]

Módulo GNSS Jamming &
  500–1.500 &
  Circuitería adicional; baja potencia \\[3pt]

Módulo Hard Kill &
  18.000–35.000 &
  Condensadores pulso $\approx 40\%$ del coste \\[3pt]

Sistema apuntado &
  3.000–6.000 &
  Servos industriales; encoder 14\,bit \\[3pt]

Unidad de control &
  2.500–5.000 &
  Pantalla táctil 7'', Ethernet/CAN \\[3pt]

Chasis y mecánica &
  2.000–4.000 &
  Anodizado, estanqueidad IP54 \\[3pt]

Alimentación &
  1.500–3.000 &
  LiPo 6S + cargador HV \\[3pt]
...
\textbf{PVP Solo Soft Kill} &
  \textbf{20.000–40.000} &
  Versión para mercado policial \\
\bottomrule
\end{tabularx}
\end{table}
