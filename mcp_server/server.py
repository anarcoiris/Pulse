...
if _MCP_OK:
    ...
    def load_preset(name: str) -> dict:
        ...
        Args:
            name: Nombre del preset. Opciones: "emp_pfn", "basic_rc", "rlc",
                  "mcu", "mcu_uart", "esp32_usb_devkit", "esp32s2_usb_devkit"
        ...
        preset_aliases = {
            "mcu_uart": "mcu_uart",
            "mcu": "mcu_uart",
            "esp32_devkit": "esp32_usb_devkit",
        }
        module_name = preset_aliases.get(name, name)
        import importlib
        try:
            mod = importlib.import_module(f"presets.{module_name}")
            graph = mod.load()
            return {
                "name": name,
                "circuit_json": json.dumps(graph.to_json()),
            ...
            return {
                "error": (
                    f"Preset '{name}' no encontrado. Opciones: emp_pfn, basic_rc, rlc, "
                    "mcu, mcu_uart, esp32_usb_devkit, esp32s2_usb_devkit"
                ),
            }
        except Exception as e:
    ...
    @mcp.tool()
    def generate_circuit_from_text(description: str, backend: str = "auto") -> dict:
        """
        ...
        Args:
            ...
            backend: "auto" | "primary" (Qwythos reasoning) | "atomic" (llama-server :11439, fast JSON)

        Returns:
            dict con circuit_json, backend usado, o mensaje de error.
        """
        from knowledge.circuit_synthesizer import CircuitSynthesizer
        synth = CircuitSynthesizer(backend=backend)
        res = synth.generate_circuit_json(description, meta={"source": "mcp"})
        if "error" in res:
            return res

        out = create_circuit_json(res["components"])
        out["backend"] = res.get("backend", backend)
        out["session_id"] = res.get("session_id")
        if res.get("log_path"):
            out["log_path"] = res["log_path"]
        return out

    @mcp.tool()
    def list_llm_backends() -> dict:
        """
        Estado de los backends LLM: primary (Ollama Qwythos) y atomic (llama-server :11439).
        Incluye perfil activo, slots paralelos, slot_ctx, y límites num_ctx / num_predict.
        """
        from knowledge.llm_backends import list_backends
        return list_backends()

    @mcp.tool()
    def get_atomic_lane_status() -> dict:
        """Estado del atomic lane (puerto, perfil, parallel, slot_ctx, modelo)."""
        from knowledge.atomic_lane import status
        return status()

    @mcp.tool()
    def set_atomic_lane_profile(
        target_profile: str,
        reason: str = "mcp",
    ) -> dict:
        """
        Cambia el perfil del atomic lane (requiere scripts en llm.backends.atomic.scripts_dir).

        Perfiles:
          concurrent2 — 2 slots × ~48k (default, maxConcurrent 2)
          concurrent3 — 3 slots × ~32k (maxConcurrent 3)
          longctx     — 1 slot × ~96k (tareas largas)

        Alias: default→concurrent2, burst→concurrent3, solo/heavy→longctx
        """
        from knowledge.atomic_lane import switch_profile
        from knowledge.llm_backends import clear_client_cache
        result = switch_profile(target_profile, reason=reason)
        clear_client_cache()
        return result
    ...
    @mcp.tool()
    def calculate_usb_diff_pair(
        target_zdiff_ohm: float = 90.0,
        substrate_height_mm: float = 1.6,
        dielectric_constant: float = 4.4,
        copper_thickness_mm: float = 0.035,
    ) -> dict:
        """
        Calcula ancho W y separación S para par diferencial USB (típico 90 ohm).

        Args:
            target_zdiff_ohm: Impedancia diferencial objetivo (USB FS/HS: 90 ohm).
            substrate_height_mm: Alto del dieléctrico (FR4: 1.6 mm).
            dielectric_constant: εr del substrato (FR4: 4.4).
            copper_thickness_mm: Grosor del cobre (1 oz = 0.035 mm).

        Returns:
            dict con W_mm, S_mm, Zdiff verificado y error porcentual.
        """
        from core.rf_tools import usb_diff_pair_dimensions
        return usb_diff_pair_dimensions(
            Zdiff_target=target_zdiff_ohm,
            h_mm=substrate_height_mm,
            er=dielectric_constant,
            t_mm=copper_thickness_mm,
        )
    ...
    def knowledge_base_stats() -> dict:
        ...
        Returns:
            dict con número de chunks por tipo, backend RAG y estado de embeddings.
        """
        return _get_kb().stats()

    @mcp.tool()
    def rebuild_embed_index() -> dict:
        """
        Rebuild dense embedding index via Ollama (nomic-embed-text by default).
        Requires Ollama running with PULSE_EMBED_MODEL pulled.
        """
        kb = _get_kb()
        return kb.rebuild_embed_index(force=True)
    ...
    def pulselab_status() -> dict:
        ...
        from knowledge.llm_backends import list_backends
        status["llm_backends"] = list_backends()
...
def main():
    ...
    parser.add_argument("--transport", default=cfg("mcp.server.transport", "stdio"),
                        choices=["stdio", "http"],
                        help="Transporte MCP (default: stdio para uso local)")
    parser.add_argument("--host", default=str(cfg("mcp.server.host", "127.0.0.1")),
                        help="Host para HTTP transport")
    parser.add_argument("--port", type=int, default=int(cfg("mcp.server.port", 8080)),
